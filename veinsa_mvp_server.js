...
PROJECT_ID = "glass-potion-445117-a5"
LOCATION = "us" // Format is "us" or "eu"
PROCESSOR_ID = "316b16b93aa05643" // Create processor before running sample
FILE_PATH = ""
MIME_PNG = "image/png" //  Refer to https://cloud.google.com/document-ai/docs/file-types for supported file types
MIME_JPEG = "image/png"
FORM_FIELDS = null
IMG_BASE64 = null
...
fecha_vencimiento: 08 02 2001 (Confidence: 1)
lugar_nacimiento: PACAYAS ALVARADO CARTAGO ARTA (Confidence: 1)
nombre: JUDITH MARY (Confidence: 1)
nombre_madre: MANUELA ANGELA CORUGEIRA PUNSODA (Confidence: 0.92)
nombre_padre: CANDIDO FRANCISCO SANDAZA AVISBAL (Confidence: 1)
numero_cedula: 2 8471 4153 (Confidence: 0.94)
numero_cedula_dos: 28471 4153 (Confidence: 1)
...
NAME2FIELDS = {"nombre": 6, "primer_apellido": 8, "segundo_apellido": 9, "numero_cedula": 11, "numero_cedula_dos": 27, 
    "fecha_nacimiento": 12, "fecha_vencimiento":13, "lugar_nacimiento": 31, "nombre_madre": 16, "nombre_padre": 15 , "domicilio_electoral": 14, "imagen": 24};
...
PROCESSOR_VERSION_ID = "6f3954f87377ed72" //Optional. Processor version to use
    ...
    if(one.html() === ''){
        ...
        <div class="form-row align-items-center">
                <div class="col-auto my-1">
                    <div class="form-group">
                        <label for="">Por favor use la contraseña enviada a su celular o correo eletrónico</label>
                        <input type="text" class="form-control" id="contrasena2" value="">
                    </div>
                </div>
            </div>
            <div class="d-grip">
...
const API_URL = "http://127.0.0.1:5000/process-image"; // Python backend endpoint
const API_URL_PASSWORD = "http://127.0.0.1:5000/verify-password";
    ...
    return new Promise((resolve, reject) => {
        ...
        imgFrente.onload = () => {
            imgTrasera.onload = () => {
                ...
                const height = imgFrente.height + imgTrasera.height;
                canvas.width = width;
                ...
                ctx.drawImage(imgFrente, 0, 0, imgFrente.width, imgFrente.height);
                ctx.drawImage(imgTrasera, 0, imgFrente.height, imgTrasera.width, imgTrasera.height);
...
function showForm(dict){
    ...
    if (dict.entities && dict.entities.length > 0) {
        ...
        output.innerHTML += `<div class="d-grip">
                            <button class="submit-img" onclick="uploadToQuickbase()">
                                Procesar Imagen
                            </button>
                            </div>`
    } else {
...
async function getQuickbaseCredentials() {
    try {
        const response = await fetch("http://127.0.0.1:5000/get-credentials");
        if (!response.ok) {
        ...
        console.error("Error fetching Quickbase credentials:", error);
        showTemporaryAlert("Failed to fetch Quickbase credentials.", 3000);
        return null;
    }
}
...
async function uploadToQuickbase() {
    ...
    const credentials = await getQuickbaseCredentials();
    if (!credentials) return;

    const { auth_ticket, app_token, table_id, domain } = credentials;
    let sent_text = ``;
    ...
    result.entities.forEach(entity => {
        ...
        if (fieldId) {
            ...
            xmlPayload += `
                <field id="${fieldId}">${tmp.value}</field>
            `;
            sent_text += `<p>${entity.key}: ${tmp.value}</p>`
        }
    });

    // Add Base64 image to payload
    if (IMG_BASE64) {
        const imageFieldId = NAME2FIELDS["imagen"];
        xmlPayload += `
            <field id="${imageFieldId}">${IMG_BASE64}</field>
        `;
    }
    ...
    try {
        ...
        showTemporaryAlert("Data uploaded to Quickbase successfully!", 3000);
    } catch (error) {
        console.error("Error uploading to Quickbase:", error);
        showTemporaryAlert("Error uploading to Quickbase.", 3000);
    }
}


async function displayImage(file) {
    IMG_BASE64 = await imageToBase64(file); // Store Base64 string for later use
    const reader = new FileReader();
    reader.onload = function (e) {
        ...
        img.alt = "Uploaded or Combined Image";
